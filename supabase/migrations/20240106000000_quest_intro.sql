-- ── Quest intro / backstory field ─────────────────────────────────────────────

alter table public.quests
  add column if not exists intro jsonb;   -- { ua, en, de } — shown before first clue

-- ── Update get_session to return quest_intro + quest_title ────────────────────
...
begin
  ...
  v_attempts_before_hint := v_quest.attempts_before_hint;
